...
# ─────────────────────────────────────────────
# MULTI-CHIPSAT EXTENSION
# ─────────────────────────────────────────────

def simulate_constellation_drift(base_inc_deg, alt_km, n_sats=4, dv_spread_ms=0.2, duration_days=5, dt_sec=30):
    """
    Simulate 4 chipsats with slightly different semi-major axes due to
    differential deployment dV. Returns list of (lats, lons) arrays.
    """
    mu = 398600.4418
    r_base = EARTH_RADIUS_KM + alt_km

    tracks = []
    for i in range(n_sats):
        # Each chipsat gets a small random dV offset → different SMA
        frac = (i - (n_sats-1)/2) / (n_sats-1)  # spread evenly: -0.5..+0.5
        dv = frac * dv_spread_ms * 0.001  # convert m/s to km/s
        # Delta-v in along-track → change in semi-major axis
        v_circ = math.sqrt(mu / r_base)
        dr = 2 * r_base * dv / v_circ   # km change in SMA (vis-viva approx)
        r_sat = r_base + dr
        alt_sat = r_sat - EARTH_RADIUS_KM

        # Slight RAAN offset from drift (simulate over time naturally)
        lats, lons, _ = ground_track(base_inc_deg, alt_sat, duration_days, dt_sec)
        # Apply a small along-track phase offset to simulate separation
        shift_steps = int(i * 300 / dt_sec)  # stagger initial phase by ~5 min each
        lats = np.roll(lats, shift_steps)
        lons = np.roll(lons, shift_steps)
        tracks.append((lats, lons, alt_sat))

    return tracks


def compute_constellation_coverage(stations, inc_deg, alt_km, duration_days, dt_sec=30, n_sats=4):
    """
    For a 4-chipsat constellation: compute total contact time (summed),
    coverage persistence (fraction of time at least 1 sat visible),
    and maximum coverage gap (seconds with zero visibility).
    """
    fp_rad = math.radians(footprint_radius_deg(alt_km, MIN_ELEVATION_DEG))
    tracks = simulate_constellation_drift(inc_deg, alt_km, n_sats, duration_days=duration_days, dt_sec=dt_sec)
    n_steps = len(tracks[0][0])

    # For each time step: is any chipsat visible to any station?
    any_visible = np.zeros(n_steps, dtype=bool)
    ...
    for (gs_lat, gs_lon, _) in stations:
        gl = math.radians(gs_lat)
        gn = math.radians(gs_lon)
        gs_any = np.zeros(n_steps, dtype=bool)

        for (lats, lons, _) in tracks:
            slr = np.radians(lats)
            snr = np.radians(lons)
            dlat = slr - gl
            dlon = snr - gn
        ...
        total_contact_steps += int(np.sum(gs_any))
        edges = np.diff(gs_any.astype(int))
        total_passes += int(np.sum(edges == 1))
        any_visible |= gs_any

    contact_hours = total_contact_steps * dt_sec / 3600.0

    # Coverage persistence: fraction of mission time at least 1 sat visible
    persistence = float(np.sum(any_visible)) / n_steps

    # Max gap: longest run of zeros in any_visible
    gaps = []
    gap = 0
    for v in any_visible:
        if not v:
            gap += 1
        else:
            if gap > 0:
                gaps.append(gap * dt_sec)
            gap = 0
    if gap > 0:
        gaps.append(gap * dt_sec)
    max_gap_min = max(gaps) / 60.0 if gaps else 0.0

    return contact_hours, total_passes, persistence * 100, max_gap_min


def plot_constellation_comparison(stations, land, coast):
    """
    Figure: single vs 4-chipsat coverage comparison across inclinations.
    Shows contact hours, coverage persistence %, and max gap for each.
    """
    print("  Computing single-sat coverage sweep...")
    single_results = []
    for inc in INCLINATIONS:
        hrs, passes = compute_coverage(stations, inc, ALTITUDE_KM, SIM_DAYS, TIME_STEP_SEC)
        single_results.append((inc, hrs, passes))

    print("  Computing 4-chipsat constellation coverage sweep...")
    const_results = []
    for inc in INCLINATIONS:
        hrs, passes, persist, max_gap = compute_constellation_coverage(
            stations, inc, ALTITUDE_KM, SIM_DAYS, TIME_STEP_SEC, n_sats=4)
        const_results.append((inc, hrs, passes, persist, max_gap))

    inclinations  = [r[0] for r in single_results]
    single_hrs    = [r[1] for r in single_results]
    const_hrs     = [r[1] for r in const_results]
    persist_pct   = [r[3] for r in const_results]
    max_gaps      = [r[4] for r in const_results]

    best_single_inc = inclinations[int(np.argmax(single_hrs))]
    best_const_inc  = inclinations[int(np.argmax(persist_pct))]
    ...
    titles = [
        f'Total contact time (hrs)\n{SIM_DAYS}-day window',
        f'Coverage persistence (%)\nFraction of time ≥1 sat visible',
        f'Max coverage gap (min)\nLongest window with 0 visibility'
    ]
    ...
    for ax, title in zip(axes, titles):
        ...
        ax.set_xlabel('Inclination (°)', color='#8b949e')
        ax.set_title(title, color='white', fontsize=9)

    # Panel 1: contact hours — single vs constellation
    axes[0].plot(inclinations, single_hrs, 'o-', color='#4fc3f7', lw=2, ms=5, label='Single chipsat')
    axes[0].plot(inclinations, const_hrs,  's-', color='#69f0ae', lw=2, ms=5, label='4-chipsat constellation')
    axes[0].axvline(best_single_inc, color='#4fc3f7', lw=1, ls='--', alpha=0.6)
    axes[0].axvline(best_const_inc,  color='#69f0ae', lw=1, ls='--', alpha=0.6)
    axes[0].legend(facecolor='#161b22', edgecolor='#30363d', labelcolor='#c9d1d9', fontsize=8)
    axes[0].set_ylabel('Hours', color='#8b949e')

    # Panel 2: coverage persistence
    cmap2  = plt.cm.RdYlGn
    norm2  = plt.Normalize(min(persist_pct), max(persist_pct))
    colors2 = [cmap2(norm2(v)) for v in persist_pct]
    axes[1].bar(inclinations, persist_pct, color=colors2, edgecolor='#30363d', width=5)
    axes[1].axvline(best_const_inc, color='#f0c040', lw=1.5, ls='--', alpha=0.8)
    axes[1].text(best_const_inc+1.5, max(persist_pct)*0.96,
                 f'Best: {best_const_inc}°', color='#f0c040', fontsize=8)
    axes[1].set_ylabel('% time covered', color='#8b949e')

    # Panel 3: max gap
    cmap3  = plt.cm.RdYlGn_r
    norm3  = plt.Normalize(min(max_gaps), max(max_gaps))
    colors3 = [cmap3(norm3(v)) for v in max_gaps]
    axes[2].bar(inclinations, max_gaps, color=colors3, edgecolor='#30363d', width=5)
    axes[2].axvline(best_const_inc, color='#f0c040', lw=1.5, ls='--', alpha=0.8)
    axes[2].set_ylabel('Minutes', color='#8b949e')

    fig.text(0.5, 0.97,
             f"Single chipsat vs 4-chipsat constellation  |  500 km  |  {SIM_DAYS}-day sim  |  "
             f"{len(stations)} TinyGS stations  |  Single best: i={best_single_inc}°  |  "
             f"Constellation best persistence: i={best_const_inc}°",
             color='#c9d1d9', fontsize=9, ha='center', va='top',
             bbox=dict(boxstyle='round,pad=0.3', facecolor='#161b22', edgecolor='#30363d'))

    path = os.path.join(OUTPUT_DIR, "constellation_vs_single.png")
    fig.savefig(path, dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close(fig)
    print(f"  ✓ {path}")
    return path, best_const_inc, best_single_inc, const_results, single_results


def plot_constellation_groundtrack(stations, best_inc, land, coast):
    """
    Figure: 4-chipsat ground tracks overlaid, color-coded per chipsat,
    showing the constellation spread over 1 day.
    """
    tracks = simulate_constellation_drift(best_inc, ALTITUDE_KM, n_sats=4,
                                          duration_days=1.0, dt_sec=30)
    colors = ['#4fc3f7', '#69f0ae', '#ff9800', '#e040fb']
    labels = ['Chipsat 1', 'Chipsat 2', 'Chipsat 3', 'Chipsat 4']

    fig, ax = plt.subplots(figsize=(16, 8), facecolor='#0d1117')
    fig.subplots_adjust(left=0.04, right=0.96, top=0.91, bottom=0.06)
    draw_world(ax, land, coast)

    for (lats, lons, alt), col, lbl in zip(tracks, colors, labels):
        split_idx = np.where(np.abs(np.diff(lons)) > 180)[0] + 1
        first = True
        for slat, slon in zip(np.split(lats, split_idx), np.split(lons, split_idx)):
            ax.plot(slon, slat, color=col, lw=0.9, alpha=0.75,
                    label=f'{lbl} ({alt-EARTH_RADIUS_KM:.1f} km)' if first else None, zorder=3)
            first = False

    gs_lats = [s[0] for s in stations]
    gs_lons = [s[1] for s in stations]
    ax.scatter(gs_lons, gs_lats, c='white', s=12, zorder=5,
               marker='^', edgecolors='#888', linewidths=0.2, alpha=0.7)

    max_lat = best_inc if best_inc <= 90 else 180 - best_inc
    ax.axhline( max_lat, color='#f0c040', lw=0.8, ls='--', alpha=0.5)
    ax.axhline(-max_lat, color='#f0c040', lw=0.8, ls='--', alpha=0.5)

    ax.legend(loc='lower left', facecolor='#161b22', edgecolor='#30363d',
              labelcolor='#c9d1d9', fontsize=9)
    ax.set_title(
        f'4-chipsat constellation ground tracks — 1-day  |  Best inclination: {best_inc}°  |  '
        f'Color = individual chipsat  |  ▲ TinyGS stations',
        color='white', fontsize=10, pad=8)

    path = os.path.join(OUTPUT_DIR, "constellation_groundtracks.png")
    fig.savefig(path, dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close(fig)
    print(f"  ✓ {path}")
    return path
...
def plot_failure_stagger(best_inc, alt_km=500.0, n_sats=4):
    """
    Figure: estimated altitude decay over time for 4 chipsats with
    slightly different ballistic coefficients, showing staggered failure times.
    Simplified atmospheric decay model (exponential density).
    """
    mu      = 398600.4418
    omega_E = 7.2921150e-5
    rho0    = 1.225e-9          # kg/km³ at sea level (simplified)
    H       = 70.0              # scale height km (rough 300-500 km average)
    Cd      = 2.2               # drag coefficient
    # Sail: circular, edge-on. Presented area varies; use small effective area
    # representing the edge-on cross section + bus
    A_base  = 0.002             # km² (edge-on effective area, ~20 cm² = 0.002 m² = 2e-9 km²)
    A_base  = 2e-9
    m_base  = 0.005             # kg (5g chipsat)

    # Each chipsat has slightly different B* (ballistic coefficient scatter ±10%)
    bc_spreads = [0.90, 0.97, 1.03, 1.10]  # relative to baseline
    colors = ['#4fc3f7', '#69f0ae', '#ff9800', '#e040fb']
    labels = [f'Chipsat {i+1}' for i in range(n_sats)]
    ...
    failure_times = []
    FAILURE_ALT = 280.0  # km — failure altitude
    dt = 300.0           # seconds per step
    t_max = 30 * 86400   # 30 days max

    for i, (bc_frac, col, lbl) in enumerate(zip(bc_spreads, colors, labels)):
        A_eff = A_base * bc_frac
        r     = EARTH_RADIUS_KM + alt_km
        alts  = [alt_km]
        times = [0.0]
        t     = 0.0

        while r - EARTH_RADIUS_KM > FAILURE_ALT and t < t_max:
            alt_cur = r - EARTH_RADIUS_KM
            rho     = rho0 * math.exp(-(alt_cur - 400) / H) * 1e9  # kg/km³
            v       = math.sqrt(mu / r)
            F_drag  = 0.5 * Cd * rho * A_eff * v**2
            a_drag  = -F_drag / m_base
            # Energy decay → SMA change
            dE_dt   = a_drag * v
            dr_dt   = 2 * r * dE_dt / v**2 / v
            r      += dr_dt * dt
            t      += dt
            alts.append(r - EARTH_RADIUS_KM)
            times.append(t / 86400.0)

        t_arr = np.array(times)
        a_arr = np.array(alts)
        fail_t = times[-1]
        failure_times.append(fail_t)

        ax.plot(t_arr, a_arr, color=col, lw=2, label=f'{lbl}  (fails ~day {fail_t:.1f})')
        ax.axhline(alts[-1], color=col, lw=0.5, ls=':', alpha=0.4)

    ax.axhline(FAILURE_ALT, color='#e74c3c', lw=1.2, ls='--', alpha=0.8)
    ax.text(0.5, FAILURE_ALT+3, f'Expected failure zone (~{FAILURE_ALT:.0f} km)', color='#e74c3c', fontsize=8)
    ax.axhline(350, color='#f0c040', lw=0.8, ls='--', alpha=0.6)
    ax.text(0.5, 353, 'TLE degrades rapidly (350 km)', color='#f0c040', fontsize=8)

    ax.set_xlabel('Mission day', color='#8b949e')
    ax.set_ylabel('Altitude (km)', color='#8b949e')
    ax.tick_params(colors='#8b949e')
    for sp in ax.spines.values(): sp.set_color('#30363d')
    ax.legend(facecolor='#161b22', edgecolor='#30363d', labelcolor='#c9d1d9', fontsize=9)

    spread = max(failure_times) - min(failure_times)
    ax.set_title(
        f'Predicted altitude decay — 4-chipsat constellation  |  '
        f'Failure spread: ~{spread:.1f} days  |  Solar activity: nominal F10.7',
        color='white', fontsize=10, pad=8)

    path = os.path.join(OUTPUT_DIR, "constellation_decay.png")
    fig.savefig(path, dpi=150, bbox_inches='tight', facecolor=fig.get_facecolor())
    plt.close(fig)
    print(f"  ✓ {path}")
    return path
...
def main_constellation():
    print("\n═══════════════════════════════════════════════════════")
    print("  4-Chipsat Constellation Extension")
    print("═══════════════════════════════════════════════════════\n")

    print("[ 1/5 ] Fetching TinyGS stations...")
    stations, live = fetch_tinygs_stations()

    print("\n[ 2/5 ] Loading world map data...")
    land, coast = get_world_geometries()

    print("\n[ 3/5 ] Single vs constellation coverage comparison...")
    p1, best_const, best_single, const_res, single_res = plot_constellation_comparison(
        stations, land, coast)

    print("\n[ 4/5 ] Constellation ground tracks...")
    p2 = plot_constellation_groundtrack(stations, best_const, land, coast)

    print("\n[ 5/5 ] Failure stagger / decay model...")
    p3 = plot_failure_stagger(best_const)

    print(f"""
─────────────────────────────────────────────────
  CONSTELLATION SUMMARY
  Single chipsat best inclination  : {best_single}°
  Constellation best inclination   : {best_const}°
  (based on coverage persistence metric)

  Key metric changes vs single sat:
  • Contact time:   ~{sum(r[1] for r in const_res)/sum(r[1] for r in single_res):.1f}x single-sat total
  • Best incl. shift: {best_single}° → {best_const}° (persistence-optimal)
...
if __name__ == "__main__":
    main()
    print("\n" + "="*55)
    print("  Running 4-chipsat constellation extension...")
    print("="*55)
    main_constellation()
